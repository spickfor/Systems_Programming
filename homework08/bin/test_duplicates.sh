...
if [ $? -ne 0 ] || [ $(grep -vc '^==' $WORKSPACE/test) -ne 1 ] || [ $(grep -v '^==' $WORKSPACE/test) -ne 0 ]; then
    error "Failure"
else
    echo "Success"
fi
...
valgrind --leak-check=full $DUPLICATES -c $ARGUMENTS &> $WORKSPACE/test
if [ $? -ne 0 ] || [ $(grep -vc '^==' $WORKSPACE/test) -ne 1 ] || [ $(grep -v '^==' $WORKSPACE/test) -ne 1 ]; then
    error "Failure"
else
    echo "Success"
fi
...
valgrind --leak-check=full $DUPLICATES -c $ARGUMENTS &> $WORKSPACE/test
if [ $? -ne 0 ] || [ $(grep -vc '^==' $WORKSPACE/test) -ne 1 ] || [ $(grep -v '^==' $WORKSPACE/test) -ne 2 ]; then
    error "Failure"
else
    echo "Success"
fi
